...
**Goal:** serve the fair-do site in **French, German, Italian** (alongside English).
...
| Adds | Translations, routing, hreflang. | Per-country **teaching-credential / tutor registration bodies**, **EUR** pricing/payouts, **local safeguarding lines**, country legal/tax, child-protection rules, support in-language. |
...
- Marketing pages: home, about, pricing, compare, for-business, faq, help, ai-tutoring, gift, styles.
...
- **User-generated content** — tutor bios/taglines, student questionnaires, messages, notes. Tutors write their own copy; never machine-translate it.
...
- **Safety + legal + safeguarding copy:** professional/qualified translation + review only — **never** unreviewed MT (§5).
...
- **Safeguarding lines are country-specific and safety-critical.** Students are often children, so the UK shows the NSPCC 0808 800 5000 / Childline 0800 1111 / 999. A French/German/Italian student or family with a safeguarding concern **must** see *their* country's lines (e.g. FR 119 Allô Enfance en Danger, DE Nummer gegen Kummer 116 111, IT Telefono Azzurro 19696 — verify current numbers). Wrong number to a child at risk is a real harm. `locale.ts` already models `CrisisLine`; extend it per locale and select by locale, not region.
...
- **No new claims** in translation — keep the same careful, plain framing (the voice rules) in every language.
...
- **Phase 2 — FR/DE/IT UI:** translate marketing + UI (MT + native review); **per-locale safeguarding lines + safety copy** (§5).
...
- **Unit:** message-key parity across `en/fr/de/it` (no missing/extra keys); `Intl` date/number/currency formatting per locale; safeguarding-line selection returns the correct country's lines for each locale.
...
- **E2E (Playwright):** switch language → nav/home/pricing render translated; `<html lang>` + hreflang correct; a safeguarding-path page shows the right safeguarding line for the locale.
...
If FR/DE/IT aren't target markets yet, this is reach beyond the UK beachhead — the same "prove the core first" logic as schools and US expansion. Worth doing when there's real demand or an SEO/marketing reason; the plan is ready to execute when that's true.
