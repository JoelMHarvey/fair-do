...
A separate, isolated instance of fair-do populated with realistic demo tutors,
ratings and availability — to show how the app looks and walk the student journey.
**Browse-only:** tutors are demo accounts (not real Stripe Connect), so booking
...
## What testers can do
- See the **public directory** (`/tutors`) full of tutors with photos, ratings, badges
- Open **tutor profiles** (bio, specialisms, reviews, availability)
- **Sign up as a student** (Clerk dev), complete the questionnaire, and get **matched** to the demo tutors
- See onboarding, the help/safeguarding pages, FAQ, blog, gift flow, etc.
...
Create a new Vercel project `fair-do-uat` from the same GitHub repo (or `vercel --prod`
...
### 5. Environment variables (Vercel → fair-do-uat → Production)
```
DATABASE_URL=postgres://…neon uat branch…
NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY=pk_test_…
CLERK_SECRET_KEY=sk_test_…
STRIPE_SECRET_KEY=sk_test_…
NEXT_PUBLIC_APP_URL=https://fair-do-uat.vercel.app   # this project's URL

ALLOW_DEMO_DATA=true        # surfaces the demo tutors (prod leaves this unset)
...
npm run seed:uat                 # seed 6 tutors, ratings, availability, demo students + org
```
...
2. **Find a tutor** → the public directory, full of tutors with photos + ★ ratings.
3. Open a profile (e.g. Priya Nair) → bio, specialisms, reviews, availability, founding badge.
4. **Get started** → sign up as a student (Clerk dev) → complete the questionnaire.
5. Land on **Your matches** → the same demo tutors, ranked to the answers.
...
- **Take UAT offline:** pause or delete the `fair-do-uat` Vercel project.

## Notes
- Demo tutors use `acct_demo_*` Stripe ids and `demo_therapist_*` / `demo_client_*`
