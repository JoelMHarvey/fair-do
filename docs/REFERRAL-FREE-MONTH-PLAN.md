...
## Reward flow
1. **Referee runs first paid lesson** → `rewardTherapistReferralOnFirstSession` (already exists): instead of marking cash owed, **increment the referrer's `freeMonthsOwed`** and set the referral `rewarded`.
...
## UI / copy
- Referral surfaces (`therapist/dashboard`, `therapist/profile`) change **"£25 per peer"** → **"a free month for every tutor you refer"** (and for free-tier: "redeemed when you upgrade").
...
- School seats: out of scope for v1 (school billing is its own plan).
...
## Testing
- Referee first lesson → referrer `freeMonthsOwed` +1 (idempotent).
