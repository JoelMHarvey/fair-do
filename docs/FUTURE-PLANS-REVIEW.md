...
Reviews the `Faresay_future/` strategy docs (master plan, scale/reinvestment, milestone pricing, client monetisation, starter tier, values copy, free→paid) against what's actually built, and against the stated roadmap. **Ordered by severity.**

**Roadmap (confirmed):** UK launch → clinics → US → Europe (**ES, IT, FR** first) → build the public **directory once >100 therapists**. The build already gates the directory behind `DIRECTORY_ENABLED` (good — matches "directory later").

---

## 🔴 Critical — money model contradicts itself

**1. The build takes a session commission; the brand says it doesn't.**
- **Build** (`src/lib/billing.ts`): Starter £0/mo **+ 2.5%**, Practice £39/mo **+ 1%**, Clinic £79/mo **+ 0%**. The commission is a real Stripe **application fee** Faresay keeps.
- **Values copy (11)** and the live pricing/marketing: *"keep 100% of every session. We don't take a percentage."*
...
- **Decision required — pick one, then make build + copy + docs all say the same thing:**
  ...
  - **(B) Keep a small commission:** then the values/marketing must stop saying "keep 100% / no percentage" and instead say "a small X% per session," everywhere.
...
- **Docs (07 / 09 / 12):** one **flat fee** (£30 → £15 floor), **lock-in for life**, **milestone step-downs** (250/500/1,000…), **free starter** until ~**3 active clients**, convert on cap with grace.
- **Build:** three fixed tiers (£0 / £39 / £79) differentiated by commission + features; Starter cap is **10 active clients**, not 3; **no lock-in**, **no milestone schedule**.
- Gaps to build for the documented model:
  ...
  - A **milestone table** (therapist-count → new-joiner rate) + logic that reads the *current* milestone rate for new signups only.
  - **Free-starter active-client cap** as a *feature boundary* (12) — currently a flat "up to 10" feature bullet, not an enforced, grace-handled cap.
  - Reconcile the **cap number**: docs say ~3, build says 10.

**3. Stale "keep 90%" in a live email.** `src/lib/email.ts` (`sendTherapistApproved`) still tells approved therapists *"you keep 90% of every session"* — pre-pivot, and contradicts **both** "keep 100%" **and** the 2.5%/1% commission. Fix the copy whichever model you pick.
...
**5. `00-MASTER-PLAN.md` is entirely pre-pivot.** It describes a *two-sided marketplace*, *"we take 15%, not 65%"*, *"founding therapists keep 90%"*, clients-first liquidity. None of that matches the current model (therapist practice tool, keep-100%/flat-fee, directory-later) **or your own roadmap.** Either rewrite it to the pivoted model or archive it — right now it's the loudest, most contradictory document in the set, and it's the one titled "master plan."
...
**9. Client monetisation (08) is well-aligned.** "Clients pay Faresay nothing by default; never a per-session %; keep options open" matches the build (clients pay the therapist via Stripe; gift vouchers exist; org credit pools exist). No change needed — just keep the payment rails flexible as the doc says (they are).
...
- Directory gated until later (`DIRECTORY_ENABLED`) — matches ">100 therapists first."
...
- Therapist owns clients / can leave with them — matches the values promise.
- Clients pay the therapist directly, no client-side platform fee — matches monetisation default.
