...
1. **Per-state child-safety + licensing nuance.** US rules vary by state: background-check/screening requirements for adults working with minors, student-data-privacy laws, and (in some states) private-school/tutoring registration. Note: private tutoring generally does **not** require a state teaching licence, so US matching is **not** necessarily state-locked the way regulated therapy is — but the data model stays region- and state-aware so we can gate where a state demands it. `[LEGAL] confirm` per state.
...
- **Separate US legal entity** (US LLC) + US bank for Stripe US payouts. Parallel to fair-do Ltd.
...
- Confirmation of whether any state private-school/tutoring licensure applies.
...
## Risks / honest notes
- Per-state child-safety/background-check rules mean national scale = repeat the verification build per state. Start narrow. (Licensure itself is usually not required for private tutors — `[LEGAL] confirm`.)
