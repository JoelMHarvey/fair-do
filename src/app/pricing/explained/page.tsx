...
export const metadata = {
  title: 'What you’ll keep — Faresay pricing in plain English',
  description: 'Exactly how Faresay pricing works: a flat monthly plan, no session commission, and Stripe’s standard processing fee — with worked examples of what lands in your bank.',
}
      ...
      <main className="bg-gradient-to-b from-brand-50 via-sand-50 to-sand-50">
        <div className="max-w-2xl mx-auto px-5 sm:px-8 py-16 sm:py-20">
          ...
          <div className="space-y-3">
            ...
            <div className="bg-white rounded-2xl border border-sand-200 p-5">
              <p className="font-medium text-brand-900">2. No session commission</p>
              <p className="text-sm text-sand-600 mt-1">
                We take <strong>no commission</strong> on your sessions — not a cut of your fee, not a referral fee,
                not a marketplace charge. You keep what you charge. The only per-session cost is Stripe’s processing
          ...
          <div className="bg-white rounded-2xl border border-sand-200 p-6">
            <p className="text-sm text-sand-600 mb-4">You charge a client <strong className="text-brand-900">{gbp0(6000)}</strong> for a session, on the <strong>Practice</strong> plan (no commission):</p>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-sand-600">Client pays</span><span className="font-medium text-brand-900">{gbp(6000)}</span></div>
              ...
              <div className="flex justify-between"><span className="text-sand-600">− Faresay commission</span><span className="text-brand-700">£0.00</span></div>
            ...
            <p className="text-xs text-sand-400 mt-4">Your {gbp0(2900)}/month plan is separate — across a typical caseload it works out to pennies per session.</p>
          </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-sand-50 text-sand-600 text-left">
                  ...
                  {EXAMPLES.map(p => <th key={p} className="px-4 py-3 font-medium text-right">{gbp0(p)} session</th>)}
                </tr>
              </thead>
              <tbody>
          ...
          <p className="text-xs text-sand-400 mt-3">
            “You keep” = the session price minus only Stripe’s processing fee (we take no commission), before your monthly plan.
            Figures are illustrative for a UK-card payment at Stripe Standard pricing (1.5% + 20p); EEA/international
            cards cost a little more. You always set your own price and keep the relationship with your client.
          </p>

          <div className="mt-12 text-center">
            <Link href="/sign-up?role=therapist" className="inline-block bg-brand-600 text-white px-7 py-3.5 rounded-full font-medium hover:bg-brand-700 transition shadow-lg shadow-brand-600/20">
              Start free
            </Link>
            <p className="text-sm text-sand-500 mt-3">Free to start · keep your own clients · cancel any time</p>
          </div>
        </div>
      </main>
      <SiteFooter />
