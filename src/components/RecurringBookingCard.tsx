...
export function RecurringBookingCard({ matchId, initial }: { matchId: string; initial: Recurring[] }) {
  ...
  async function add() {
    ...
    if (res.ok) {
      ...
      const d = await res.json().catch(() => ({})); setError(d.error ?? 'Could not set up.')
    }
    setBusy(false)
  }
  ...
  return (
    <div>
        ...
        <ul className="space-y-2 mb-4">
          {items.map(r => (
            <li key={r.id} className="flex items-center justify-between gap-3 bg-white rounded-xl border border-sand-200 px-4 py-3">
              <div>
                <p className="text-sm font-medium text-sand-900">Every {DAYS[r.dayOfWeek]} at {r.startTime}</p>
                <p className="text-xs text-sand-400">
                  {r.durationMins} min ·{' '}
                  {!r.hasCard ? <span className="text-amber-600">waiting for student to add a card</span>
                    : r.active ? <span className="text-brand-700">active</span>
                    : <span className="text-sand-500">paused</span>}
                </p>
              </div>
              <div className="flex items-center gap-3 shrink-0">
                {r.hasCard && (
                  <button onClick={() => toggle(r)} className="text-sm text-sand-600 hover:text-brand-700">{r.active ? 'Pause' : 'Resume'}</button>
                )}
                <button onClick={() => remove(r.id)} className="text-sm text-sand-400 hover:text-red-500">Cancel</button>
              </div>
            </li>
          ))}
        </ul>
      ...
      <p className="text-xs text-sand-400 mt-2">The student gets a prompt to authorise a card. We then create and charge each lesson automatically.</p>
    </div>
  )
}
